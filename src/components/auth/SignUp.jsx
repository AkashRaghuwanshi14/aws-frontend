...
const SignUp = () => {
 ...
 const handlesubmit=async(e)=>{
     e.preventDefault();
      try{ 
           setloading(true);
          let res=await axios.post("54.199.96.137:3000/api/user/signup",{
              email:email,
              username:user,
              password:password
          })
 ...
 <div className='flex flex-col gap-2 justify-center items-center'>
          ...
          <input onChange={(e)=>{setuser(e.target.value)}} 
             value={user}
          className='text-white m-auto px-6 py-1  border-2  border-white rounded-sm outline-none' type='text' id='username' name='username' autoComplete='off' placeholder='Enter Your UserName'></input>
      </div>
       ...
       <div className='flex justify-center mt-3'>
        ...
        <button disabled={loading}
         className="bg-sky-500  hover:bg-sky-700 ... px-16 py-1 rounded-sm  text-white">{loading ? "loading.." :"Signup"}</button>
      </div>
     ...
     <div className='bg-black py-8 w-1/4 mt-3 flex justify-center rounded-sm border-2 border-white'>
       <p className='text-white'>Already Have Account?<Link to="/login" className='text-blue-700'>Signin</Link></p>
     </div>
